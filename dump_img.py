...
for pid in xrange(proc.storage_status_array().size()):
    ...
    for label in ['data','weight','label']:

        batch_data = larcv.BatchDataStorageFactory('float').get().get_storage(label).get_batch(pid)
        ...
        for entry,entry_pydata in enumerate(pydata):
            entry_pydata = entry_pydata.reshape(pydim[1:3]).astype(np.float32)
            print entry, entry_pydata.shape
            fig,ax=plt.subplots(figsize=(12,8),facecolor='w')
            fmappng=plt.imshow(entry_pydata,cmap='jet',interpolation='none')
            fmappng.write_png('%s_Proc%02d_BatchEntry%03d.png' % (label,pid,entry))
            plt.close(fig)
